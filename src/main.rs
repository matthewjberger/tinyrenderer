...
// First attempt
#[allow(dead_code)]
fn line_1(x0: i32, y0: i32, x1: i32, y1: i32, img: &mut RendererImage, color: RendererColor) {
    let resolution = 100;
...
// Second attempt
#[allow(dead_code)]
fn line_2(x0: i32, y0: i32, x1: i32, y1: i32, img: &mut RendererImage, color: RendererColor) {
    for x in x0..=x1 {
...
// Third attempt
#[allow(dead_code)]
fn line_3(x0: i32, y0: i32, x1: i32, y1: i32, img: &mut RendererImage, color: RendererColor) {
    ...
    for x in x0..=x1 {
        let t = (x - x0) as f32 / (x1 - x0) as f32;
        let mut y = (y0 as f32 * (1.0 - t) + y1 as f32 * t) as i32;
        let mut x = x;
        if steep {
...
// Fourth attempt
#[allow(dead_code)]
fn line_4(x0: i32, y0: i32, x1: i32, y1: i32, img: &mut RendererImage, color: RendererColor) {
    let (mut x0, mut y0, mut x1, mut y1) = (x0, y0, x1, y1);
    let steep = if (x0 - x1).abs() < (y0 - y1).abs() {
        mem::swap(&mut x0, &mut y0);
        mem::swap(&mut x1, &mut y1);
